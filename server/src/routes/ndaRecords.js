...
import { NDA_ACTION_FIELD, advanceNdaRecord } from "../lib/ndaActions.js";
...
ndaRecordsRouter.post("/:id/:action", asyncHandler(async (req, res) => {
  const step = NDA_ACTION_FIELD[req.params.action];
  ...
  const { record, emailResult } = await advanceNdaRecord(existing, req.params.action);
