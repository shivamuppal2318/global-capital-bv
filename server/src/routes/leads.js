...
leadsRouter.post("/bulk", asyncHandler(async (req, res) => {
  ...
  for (const [index, leadInput] of parsed.data.leads.entries()) {
    try {
      const lead = await prisma.lead.create({ data: { ...leadInput, campaignId: campaign.id } });
      const scheduledCount = await scheduleCadenceSteps(lead, campaign.cadenceSteps);
...
leadsRouter.get("/:id/activity", asyncHandler(async (req, res) => {
...
leadsRouter.post("/:id/send", asyncHandler(async (req, res) => {
...
leadsRouter.post("/:id/send-template", asyncHandler(async (req, res) => {
...
leadsRouter.post("/:id/simulate-reply", asyncHandler(async (req, res) => {
...
// automatic.
leadsRouter.post("/:id/mark-call-completed", asyncHandler(async (req, res) => {
  const lead = await prisma.lead.findUnique({ where: { id: req.params.id } });
  if (!lead) {
    return res.status(404).json({ error: "Lead not found" });
  }
  ...
  await prisma.$transaction([
    prisma.lead.update({ where: { id: lead.id }, data: { callCompletedAt: completedAt } }),
...
leadsRouter.post("/:id/schedule-cadence", asyncHandler(async (req, res) => {
  if (!isQueueEnabled()) {
