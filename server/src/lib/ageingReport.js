...
async function interestedAgeing(directLeadWhere) {
  const leads = await prisma.lead.findMany({
    where: { status: { in: ["INTERESTED", "QUALIFIED", "NEGOTIATION", "CONVERTED"] }, ...directLeadWhere },
...
async function dataRoomAgeing(leadWhere) {
  const signedNdas = await prisma.ndaRecord.findMany({
    where: { status: "SIGNED", ...leadWhere },
...
export async function computeAgeingReport(channelPartner = null) {
  ...
  const [openOutreachLeads, openInterestedLeads, openNdaRecords, dataRoomDeals, openIoiRecords, fieldVisitStageRecords, termSheetStageRecords, meetingResults, staleInterested] = await Promise.all([
    ...
    interestedAgeing(directLeadWhere),
