// Mirrors the mock data hardcoded in ../src/data/crmData.js and the
// repliedLeads seed in ../src/App.jsx, so a freshly provisioned backend
// tells the same demo story the frontend already shows standalone.
...
async function main() {
  ...
  // Matches the default templateDrafts hardcoded in ../src/App.jsx exactly,
  // so a freshly seeded backend and a never-touched frontend agree on
  // subject/body by default. html left null so each renders through the
  // default branded wrapper in src/lib/renderTemplate.js.
  const templatesSeed = [
    {
      key: "interested",
  ...
  console.log(`Seeded campaign "${campaign.name}" with ${leadsSeed.length} leads and ${templatesSeed.length} templates.`);
}
